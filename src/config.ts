...
function validateConfig(c: Config, configPath: string): void {
  ...
  if (!c.api?.baseUrl || typeof c.api.baseUrl !== "string")
    missing.push("api.baseUrl");
  if (typeof c.api?.timeoutMs !== "number" || c.api.timeoutMs <= 0)
    missing.push("api.timeoutMs (positive number)");
  if (!Array.isArray(c.s3?.buckets)) missing.push("s3.buckets (array)");
  if (!c.s3?.stagingDir || typeof c.s3.stagingDir !== "string")
    missing.push("s3.stagingDir");
  if (!c.s3?.region || typeof c.s3.region !== "string")
    missing.push("s3.region");
  if (!c.run?.checkpointPath || typeof c.run.checkpointPath !== "string")
    missing.push("run.checkpointPath");
  if (typeof c.run?.concurrency !== "number" || c.run.concurrency < 1)
    missing.push("run.concurrency (>= 1)");
  if (!c.logging?.dir || typeof c.logging.dir !== "string")
    missing.push("logging.dir");
  if (!c.report?.outputDir || typeof c.report.outputDir !== "string")
    missing.push("report.outputDir");
  if (!Array.isArray(c.report?.formats) || c.report.formats.length === 0)
    missing.push("report.formats (non-empty array)");
  if (c.report?.retainCount !== undefined) {
    if (
      typeof c.report.retainCount !== "number" ||
      c.report.retainCount < 0 ||
      !Number.isInteger(c.report.retainCount)
    ) {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Invalid config at ${configPath}. Missing or invalid: ${missing.join(", ")}.`,
    );
  }
}
...
export function loadConfig(configPath: string = CONFIG_PATH): Config {
  ...
  if (envBucket && envTenantPurchasers) {
    try {
      const mapping = JSON.parse(envTenantPurchasers) as Record<
        string,
        string[]
      >;
  ...
  if (process.env.INTELLIEXTRACT_BASE_URL)
    withEnv.api.baseUrl = process.env.INTELLIEXTRACT_BASE_URL;
  const envConfig = withEnv as unknown as Record<string, unknown>;
  if (process.env.INTELLIEXTRACT_ACCESS_KEY !== undefined)
    envConfig.accessKey = process.env.INTELLIEXTRACT_ACCESS_KEY;
  if (process.env.INTELLIEXTRACT_SECRET_MESSAGE !== undefined)
    envConfig.secretMessage = process.env.INTELLIEXTRACT_SECRET_MESSAGE;
  if (process.env.INTELLIEXTRACT_SIGNATURE !== undefined)
    envConfig.signature = process.env.INTELLIEXTRACT_SIGNATURE;
  return withEnv;
}
