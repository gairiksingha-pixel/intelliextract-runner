    ...
    .map(
      ({ name, status, duration }) => `
              <tr style="border-bottom:#e4e4e7 1px solid;">
                <td style="padding:12px 8px; font-size: 13px; color: #334155;">${name}</td>
                <td style="padding:12px 8px;color:${getColorForStatus(
                  status,
                )};font-weight:bold; font-size: 13px;">${status}</td>
                <td style="padding:12px 8px; font-size: 13px; color: #64748b;">${duration}</td>
              </tr>`,
    )
    .join("");
